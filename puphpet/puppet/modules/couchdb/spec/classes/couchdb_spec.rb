...
describe 'couchdb', :type => :class do
  let(:title) { 'couchdb' }
  let(:params) do {
      :download   => 'http://apache.mirrors.ovh.net/ftp.apache.org//dist/couchdb/source/1.6.1/apache-couchdb-1.6.1.tar.gz',
      :filename   => 'apache-couchdb-1.6.1.tar.gz',
      :foldername => 'apache-couchdb-1.6.1',
    }
  end

  it do
    should contain_exec('download').with({
      'command' => '/usr/bin/wget -q http://apache.mirrors.ovh.net/ftp.apache.org//dist/couchdb/source/1.6.1/apache-couchdb-1.6.1.tar.gz -O apache-couchdb-1.6.1.tar.gz',
      'cwd'     => '/usr/local/src',
    })
  end
  ...
  it do
    should contain_exec('extract').with({
      'command' => '/bin/tar -xzvf apache-couchdb-1.6.1.tar.gz',
      'cwd'     => '/usr/local/src',
    })
  end
  ...
  it do
    should contain_exec('configure').with({
      'cwd' => '/usr/local/src/apache-couchdb-1.6.1',
    })
  end
